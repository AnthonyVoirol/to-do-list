...
switch ($method) {
    ...
    case 'PUT':
        ...
        if (isset($data['isDone'])) {
            ...
            $stmt = $conn->prepare('UPDATE tasks SET isDone = ? WHERE id = ? AND user_id = ?');
            $stmt->bind_param('iii', $isDone, $task_id, $user_id);
            $stmt->execute();
        ...
        if ($stmt->affected_rows > 0) {
            ...
            http_response_code(500);
            echo json_encode(['error' => 'Failed to update task or task not found.']);
        }
